...
void con_select_char(int cx, int cy)
{
    ...
    xtk_color_t bgcolor = (0xffffff - (con_screen.background_color & 0xffffff)) | (0xff << 24);
    xtk_color_t fontcolor = (0xffffff - (con_screen.font_color & 0xffffff)) | (0xff << 24);
...
void con_set_back_color(xtk_color_t color)
{
    con_screen.background_color = color;
}

void con_set_font_color(xtk_color_t color)
{
...
int con_get_key(int kcode, int kmod)
{
    ...
    if (kmod & UVIEW_KMOD_CTRL) {
        if (kcode == UVIEW_KEY_UP) {
            scroll_screen(CON_SCROLL_UP, 1, 0, 1);
            return 0;
        } else if (kcode == UVIEW_KEY_DOWN) {
            scroll_screen(CON_SCROLL_DOWN, 1, 0, 1);
            return 0;
        } else if (kcode == UVIEW_KEY_c || kcode == UVIEW_KEY_C) { /* ctrl + C */
            char buf[2] = {0, 0};
            buf[0] = '\003';
            con_screen.outs("^C");
            write(ft_pty.fd_master, buf, strlen(buf));
            return 0;
        }
    }
...
int init_console()
{
    ...
    sh_window_rect_fill(0, 0, con_screen.width, con_screen.height, CON_SCREEN_BG_COLOR);
