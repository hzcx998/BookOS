...
void cursor_pos_check()
{
	...
	//如果光标向右移动超出，就切换到下一行
	if (cursor.x > con_screen.columns - 1) {
        /* 如果行超出最大范围，就回到开头 */
        if (cursor.y < con_screen.rows) {
            //如果y 没有到达最后一行，就移动到行首
			cursor.x = 0;
		} else {
			//如果y到达最后一行，就移动到行尾
...
void move_cursor(int x, int y)
{
	...
	//文字颜色
    con_ouput_visual(ch, cursor.x * con_screen.char_width,
        cursor.y * con_screen.char_height);
...
void draw_cursor()
{
    ...
    case CS_SOLID_FRAME:
        ...
        break;
    case CS_HOLLOW_BOX:
        sh_window_rect(x, y, cursor.width, cursor.height, cursor.color);
        break;
    case CS_VERTICAL_BAR:
        ...
        break;
    case CS_UNDERLINE:
        sh_window_rect_fill(x, y + con_screen.char_height - 1,
            cursor.width, cursor.height, cursor.color);
        break;
    default:
        break;
...
void focus_cursor()
{
    if (!cursor.visual) {   /* 没有显示 */
        ...
        } else if (cursor.y >= con_screen.rows) { /* 光标在顶部 */
            scroll_screen(CON_SCROLL_DOWN, cursor.y - con_screen.rows + 1, 0, 1);
        }
    }
}
