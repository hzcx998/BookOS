...
void _vt100_reset(struct vt100 *term) {
	...
	term->fill_rect(term, 0, 0, term->width, term->height);
}
...
void _vt100_clearLines(struct vt100 *term, uint16_t start_line, uint16_t end_line) {
	term->fill_rect(term, 0, start_line, term->width, end_line - start_line + 1);
}
...
void _vt100_drawCursor(struct vt100 *t, int onshow) {
    ...
    t->draw_cursor(t, t->cursor_x, t->cursor_y, t->char_width, t->char_height,
        onshow > 0 ? t->front_color : t->back_color);
}
				...
				switch (arg) {
					...
					case 'K': { // clear line from cursor right/left
						if (term->narg == 0 || (term->narg == 1 && term->args[0] == 0)) {
							...
							// including cursor
							term->fill_rect(term, term->cursor_x, term->cursor_y, term->width - term->cursor_x, 1);
						} else if (term->narg == 1 && term->args[0] == 1) {
							// clear from left to current cursor position
							term->fill_rect(term, 0, term->cursor_y, term->cursor_x + 1, 1);
						} else if (term->narg == 1 && term->args[0] == 2) {
							// clear whole current line
							term->fill_rect(term, 0, term->cursor_y, term->width, 1);
						}
						term->state = _st_idle;
						break;
					}
		...
		case EV_CHAR: {
			switch (arg) {
				...
				case '\b': { // backspace 0x08
                    ...
                    term->draw_char(term, term->cursor_x,
					    term->cursor_y, ' ');
                    _vt100_drawCursor(term, 1);
					break;
				}
